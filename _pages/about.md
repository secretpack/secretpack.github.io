---
title: "Who Am I"
permalink: /about/
layout: single
author_profile: true
toc: true
toc_label: "목차"
toc_icon: "user"
toc_sticky: true
---

**Security Researcher** in 42dot inc.  

**Interested in :**
* Pwning / Penetration Testing (Target : Automotive/IoT/Application)
* Reverse Engineering
* Hardware Security Module (HSM) Security
* ARM Trust Execution Environment (TEE) Security
* Cybersecurity Mangement & Threat Modeling 

## Technical Stack  
| Subject | Details |
| ----- | ----- |
| Automotive Security | CAN/CAN-FD, DoIP, SOME/IP, Bluetooth, WiFi, RF, Cellular Fuzzing & Penetration Testing |
| Reverse Engineering | Use IDA, Ghidra, WinDBG, Radare2, GDB with plugins (gef, pwndbg), lldb for IOS, Trace32 Debugger for Automotive |
| Fuzzing Research | Use AFL++, WinAFL, Syzkaller, Defensics for Vulnerability Analysis & Penetration Testing |
| LLM Skills | Use my own skills for...|

## 📄 Paper / 🏆 Award / 📜 Certification
| 📄 Paper Name | Journal | Link |
| ----- | ----- | ----- |
|Analyzing Attack Scenarios against Open webOS-based Smart Devices| CISC W'16 | [link](https://syscore.sejong.ac.kr/~woongbak/publications/DC10_11_12.pdf)
|Artifact Analysis of SMART TV based on webOS | Journal of Digital Forensic Korea | [link](https://www.dbpia.co.kr/journal/articleDetail?nodeId=NODE11083152) |  

| 🏆 Award Name | Description | Rank |
| ----- | ----- | ----- |
| LG Certificate of Appreciation | Received a Letter of Appreciation from LG Electronics for vulnerability discovery. | - | 
| 2022 SSTF | Samsung Security Technical Forum CTF | 5th palace|
| 2025 VCC | Block Harvor + VicOne hosted Vehicle Cybersecurity Competition | Individial Winner |
| 2025 Automotive SW AI Attack/Analysis/Defense Competition | SCH University hosted Vehicle Cybersecurity Competition | 1st palace | 

| 📜 Certificate | Organization |
| ----- | ----- |
| ISO/SAE 21434 CSMS Lead Auditor | BSI Group |

## Contact

**✉️ Email** : secretpack97@gmail.com  
**GitHub** : [github.com/secretpack](https://github.com/secretpack)  
<i class="fab fa-fw fa-linkedin" aria-hidden="true"></i> **Linkedin** : [MY Linked in Page](https://www.linkedin.com/in/gyeongsik-song-490b8128b/)